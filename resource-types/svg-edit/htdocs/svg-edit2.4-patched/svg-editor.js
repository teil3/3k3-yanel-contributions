...
function svg_edit_setup() {
	...
	var default_img_url =  scriptbase + "images/logo.png";
	...
	(function() {
		...
		var dbox = function(type, msg, callback, defText) {
			$('#dialog_content').html('<p>'+msg.replace(/\n/g,'</p><p>')+'</p>')
				.toggleClass('prompt',(type=='prompt'||type=='lookup'));
			btn_holder.empty();
			...
			if(type == 'lookup') {
				var input = $('<input type="text" style="width:180px;display:inline;" id="lookup_path_promt">');
				input.val(defText || '');
				lookup_box = $('<div style="display:block;"></div>');
				browse_btn = $('<button type="button" onclick="openLookup(\'lookup_path_promt\')">Browse</button>');
				input.appendTo(lookup_box);
				browse_btn.appendTo(lookup_box);
				lookup_box.prependTo(btn_holder);
				input.bind('keydown', {combi:'return'}, function() {ok.click();});
			}
			...
			ok.click(function() { 
				box.hide();
				var resp = (type == 'prompt'||type == 'lookup')?input.val():true;
		...
		$.lookup = function(msg, txt, cb) { dbox('lookup', msg, cb, txt);};
	}());
...
// 				});
	...
	var setIconSize = function(size) {
		...
		var cssResizeRules = {
			...
			"input.spin-button": {
				'background-image': {l: "url('" + scriptbase + "images/spinbtn_updn_big.png')", xl: "url('" + scriptbase + "images/spinbtn_updn_big.png')"},
	...
	function promptImgURL() {
		$.lookup(uiStrings.enterNewImgURL, default_img_url, function(url) {
	...
	var colorPicker = function(elem) {
		...
		$("#color_picker")
			...
			{ 
				...
				images: { clientPath:  scriptbase + "jgraduate/images/" }
			},
			function(p) {
	...
	setTimeout(function() {
		svgCanvas.embedImage(scriptbase + 'images/logo.png', function(datauri) {
			if(!datauri) {
				// Disable option
...
//	var revnums = "svg-editor.js ($Rev: 1213 $) ";
...
(function() {
	$.svgIcons(scriptbase + 'images/svg_edit_icons.svg', {
		w:24, h:24,
		id_match: false,
		no_img: true,
		fallback_path: scriptbase + 'images/',
		fallback:{
			'new_image':'clear.png',
			'exit':'exit.png',
			'save':'save.png',
