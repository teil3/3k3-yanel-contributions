/*/*
 * Copyright 2014 litwan
 */

package com.litwan.yanel.impl.resources.jqueryxmleditor;
...
public class JqueryXmleditorResource extends ExecutableUsecaseResource {
    ...
    private static Logger log = Logger.getLogger(JqueryXmleditorResource.class);
    ...
    public boolean checkPreconditions() throws UsecaseException {
        ...
        if (ResourceAttributeHelper.hasAttributeImplemented(resToEdit, "Viewable", "2")) {
            try {
                ...
                if (!view.getMimeType().contains("xml")) {
                    addError("Mime-Type not supported: " + view.getMimeType() + ". Only edit html documents with xmleditor. ");
                    return false;
                }
            } catch (Exception e) {
...
//        addInfoMessage("Content cleaned with tidy. ");
    ...
    public String GetXSDFilename() {
        String xsdName = getPath();
        xsdName = xsdName.substring(xsdName.lastIndexOf("/")+ 1);
        xsdName = xsdName.substring(0,xsdName.indexOf("."));
        log.warn("xsdName expected " + xsdName);
        return xsdName;
    }
